import { createContext, useContext, useState, useMemo, useEffect } from 'react';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useGames } from './GamesContext';
import { syncReminderSchedule, scheduleWeeklyDigest, cancelWeeklyDigest } from './notifications';
import { resolvedWatchlistCount } from './watchlistUtils';
import { maybeRequestReview } from './reviewPrompt';
import { PLATFORMS } from './theme';

const STORAGE_KEY = 'watchlist_state_v1';
// Every platform key the app understands — used as the "saved for all
// platforms" default whenever a game is watchlisted with no specific
// platform in context (e.g. tapping the Calendar hero's button, or the
// detail screen's CTA when arriving with no filter), and to migrate
// pre-existing watchlist entries from before per-platform tracking existed
// (see the load effect below) so they keep showing exactly as they did
// before, on every platform, until the user explicitly re-saves them under
// one specific platform going forward.
const ALL_PLATFORM_KEYS = Object.keys(PLATFORMS);

const WatchlistContext = createContext(null);

export const LEAD_OPTIONS = [
  { key: 'release_day', label: 'Release Day', days: 0 },
  { key: '3_days', label: '3 Days Before', days: 3 },
  { key: '1_week', label: '1 Week Before', days: 7 },
];

export function WatchlistProvider({ children }) {
  const { games } = useGames();
  // The real source of truth for what's watchlisted: title -> array of
  // platform keys it's saved under. Replaces the old plain "is this title
  // saved, yes/no" model — a game wishlisted on PS5 should not read as
  // wishlisted when browsing/filtering Xbox for the same game unless that
  // was done specifically too. `saved` below is derived from this for every
  // existing consumer that only cares "is this title saved for anything".
  const [savedPlatforms, setSavedPlatforms] = useState({});
  const [reminders, setReminders] = useState({});
  // Which platform a title's single reminder should track the release date
  // of — set to whichever platform it was most recently explicitly added
  // under (see toggleWatchlist below). Originally existed only to pick a
  // badge to display (fix log item 4); that job now belongs to
  // `savedPlatforms` itself, so this field's only remaining purpose is
  // feeding syncReminderSchedule/scheduleWeeklyDigest the right per-platform
  // date when one exists (see lib/notifications.js).
  const [platformContext, setPlatformContext] = useState({});
  // ADDED (item 40 — keep released games visible in Watchlist for 24h): the
  // backend's own games query is forward-looking only (see
  // gaming-views-backend/api/games.js) — the instant a game's release date
  // passes, it simply stops coming back, and `games.find(...)` for that
  // title resolves to nothing. Rather than widening that backend contract
  // (which Calendar, Search, and the hero carousel all also rely on staying
  // upcoming-only), this keeps the last real game data seen for each
  // currently-saved title, title -> full game object, so the Watchlist
  // screen has something to render for a brief window after the backend
  // stops providing it. Kept in sync with live data below and pruned the
  // moment a title is actually un-watchlisted (see toggleWatchlist) — not
  // meant to accumulate stale entries for titles no longer saved at all.
  const [savedGameSnapshots, setSavedGameSnapshots] = useState({});
  // ADDED (item 41 — Add to Calendar button state fix): title -> true once
  // addGameReleaseToCalendar has actually succeeded for that title (see
  // app/game/[title].js). Deliberately NOT the created event's own ID —
  // that's still out of scope, same as when the calendar feature first
  // shipped — just enough to know "don't create a second one" and show a
  // permanent confirmed state instead of the local, transient
  // idle/adding/denied cycle reverting after a few seconds regardless of
  // outcome (the actual bug this fixes). Unlike savedGameSnapshots above,
  // deliberately NEVER pruned when a title leaves the watchlist — a real
  // calendar event, once created, still exists whether or not the game
  // stays watchlisted, so the "don't create a duplicate" guard has to
  // survive a remove-then-re-add of the same title too.
  const [calendarAdded, setCalendarAdded] = useState({});
  // UPDATED (multi-select platform/genre filter): these used to be a single
  // string each ('all' meaning no filter, otherwise exactly one key/genre) —
  // onboarding could only ever turn a pick into a real filter when exactly
  // one platform/genre was selected, silently applying no filter at all for
  // 2+ picks. Now a plain array — an empty array is the new "no filter"
  // (equivalent to the old 'all'), any number of entries filters to all of
  // them. See the load effect below for the one-time migration of existing
  // installs' old single-string values into this shape.
  const [preferredPlatforms, setPreferredPlatforms] = useState([]);
  const [preferredGenres, setPreferredGenres] = useState([]);
  const [weeklyDigestEnabled, setWeeklyDigestEnabled] = useState(false);
  const [hydrated, setHydrated] = useState(false);

  // Load persisted state once on mount. Everything here used to live only
  // in memory and reset on every app restart — this is the fix for that.
  useEffect(() => {
    (async () => {
      try {
        const raw = await AsyncStorage.getItem(STORAGE_KEY);
        if (raw) {
          const parsed = JSON.parse(raw);
          if (parsed.savedPlatforms && typeof parsed.savedPlatforms === 'object') {
            setSavedPlatforms(parsed.savedPlatforms);
          } else if (Array.isArray(parsed.saved)) {
            // Pre-existing data from before per-platform tracking existed —
            // every entry becomes "saved for every platform" so it keeps
            // showing exactly as it did before, on every screen/filter,
            // until the user explicitly re-saves it under one platform.
            const migrated = {};
            for (const title of parsed.saved) migrated[title] = ALL_PLATFORM_KEYS;
            setSavedPlatforms(migrated);
          }
          if (parsed.reminders && typeof parsed.reminders === 'object') setReminders(parsed.reminders);
          // Existing installs' persisted state won't have this field yet —
          // the `useState({})` default above already covers that, this just
          // avoids overwriting it with something invalid if it's ever there.
          if (parsed.platformContext && typeof parsed.platformContext === 'object') setPlatformContext(parsed.platformContext);
          // Existing installs won't have this field yet — the useState({})
          // default above already covers that.
          if (parsed.savedGameSnapshots && typeof parsed.savedGameSnapshots === 'object') setSavedGameSnapshots(parsed.savedGameSnapshots);
          if (parsed.calendarAdded && typeof parsed.calendarAdded === 'object') setCalendarAdded(parsed.calendarAdded);
          // One-time migration for installs persisted before the
          // multi-select change: the old fields were a single string each
          // ('all' or one specific value). Real fields confirmed in this
          // same STORAGE_KEY blob, not separate AsyncStorage keys — there
          // was never more than one string per axis to carry over.
          // Untouched by this migration if the new array fields are already
          // there (a later install, or one that's already gone through it
          // once — an empty array is a real, valid "no filter" persisted
          // value in its own right, not an absent one, hence checking with
          // `||` rather than only running when truthy-and-non-empty).
          if (parsed.preferredPlatforms || parsed.preferredPlatform) {
            setPreferredPlatforms(
              Array.isArray(parsed.preferredPlatforms)
                ? parsed.preferredPlatforms
                : (parsed.preferredPlatform && parsed.preferredPlatform !== 'all' ? [parsed.preferredPlatform] : [])
            );
          }
          if (parsed.preferredGenres || parsed.preferredGenre) {
            setPreferredGenres(
              Array.isArray(parsed.preferredGenres)
                ? parsed.preferredGenres
                : (parsed.preferredGenre && parsed.preferredGenre !== 'all' ? [parsed.preferredGenre] : [])
            );
          }
          if (typeof parsed.weeklyDigestEnabled === 'boolean') setWeeklyDigestEnabled(parsed.weeklyDigestEnabled);
        }
      } catch {
        // Corrupt/unreadable storage shouldn't crash the app — just start fresh.
      } finally {
        setHydrated(true);
      }
    })();
  }, []);

  // Persist on every change, but only once initial hydration has finished —
  // otherwise the very first render (before loading finishes) would
  // immediately overwrite real saved data with empty defaults.
  useEffect(() => {
    if (!hydrated) return;
    const payload = JSON.stringify({
      savedPlatforms,
      reminders,
      platformContext,
      savedGameSnapshots,
      calendarAdded,
      preferredPlatforms,
      preferredGenres,
      weeklyDigestEnabled,
    });
    AsyncStorage.setItem(STORAGE_KEY, payload).catch(() => {});
  }, [hydrated, savedPlatforms, reminders, platformContext, savedGameSnapshots, calendarAdded, preferredPlatforms, preferredGenres, weeklyDigestEnabled]);

  // "Is this title saved at all, for any platform" — every existing
  // consumer (badge counts, the Calendar's hide-if-watchlisted filter, the
  // Watchlist screen's own list) only ever needed this coarse view, so it
  // stays a plain Set derived from savedPlatforms rather than needing every
  // one of those call sites rewritten.
  const saved = useMemo(
    () => new Set(Object.keys(savedPlatforms).filter((t) => (savedPlatforms[t] || []).length > 0)),
    [savedPlatforms]
  );

  // Keep real scheduled local notifications in sync with the watchlist and
  // each game's lead-time choice, once game data is actually available.
  // platformContext lets a title's one reminder track the release date of
  // the specific platform it was added under, when that differs from the
  // game's other platforms (see lib/notifications.js).
  useEffect(() => {
    if (!hydrated || games.length === 0) return;
    syncReminderSchedule(saved, reminders, games, LEAD_OPTIONS, platformContext).catch(() => {});
  }, [hydrated, saved, reminders, games, platformContext]);

  // ADDED (item 40 — keep released games visible in Watchlist for 24h):
  // keeps savedGameSnapshots current with real live data for every
  // currently-saved title, whenever the live dataset actually has it — this
  // runs on every `games` refresh, not just once at add-time, so a snapshot
  // reflects whatever was most recently seen (e.g. a corrected cover image)
  // right up until the backend stops returning the title at all. Bails out
  // without writing state when nothing's actually changed, since `games`
  // refetches (and this effect re-running) happen far more often than any
  // snapshot's contents genuinely differ.
  useEffect(() => {
    if (!hydrated || games.length === 0) return;
    setSavedGameSnapshots((prev) => {
      let changed = false;
      const next = { ...prev };
      for (const title of saved) {
        const live = games.find((g) => g.title === title);
        if (live && next[title] !== live) {
          next[title] = live;
          changed = true;
        }
      }
      return changed ? next : prev;
    });
  }, [hydrated, saved, games]);

  // Same idea for the Weekly Digest toggle.
  // FIXED 2026-09-08 (item 31): this used to schedule the digest whenever
  // the toggle was on, full stop — scheduleWeeklyDigest() itself always
  // scheduled a real notification, just with body text that read "Nothing
  // from your watchlist is releasing this week" when the watchlist was
  // empty. That's exactly the reported bug (the digest still appearing with
  // nothing wishlisted) — the toggle being on was never meant to promise a
  // notification every Monday regardless of whether there's anything to
  // report. Now checked against resolvedWatchlistCount (the same "does this
  // title still resolve to a real, current game" helper item 9 already
  // built for the header/badge counts, not just the raw saved.size) —
  // zero real watchlisted titles cancels/skips scheduling entirely, same as
  // the toggle being off. Re-evaluated on every relevant change (saved,
  // games) via the existing dependency array, so a digest correctly starts
  // scheduling again the moment something real gets watchlisted.
  useEffect(() => {
    if (!hydrated || games.length === 0) return;
    if (weeklyDigestEnabled && resolvedWatchlistCount(saved, games) > 0) {
      scheduleWeeklyDigest(saved, games, platformContext).catch(() => {});
    } else {
      cancelWeeklyDigest().catch(() => {});
    }
  }, [hydrated, weeklyDigestEnabled, saved, games, platformContext]);

  // `platform` — the specific platform to toggle, when the caller has one
  // (GameCard/detail screen pass their active filter, e.g. `highlightPlatform`
  // — undefined on paths with no active filter). `gamePlatforms` — the
  // game's own current platform list, used only as the "add every platform"
  // default when there's no specific platform in context (see below).
  const toggleWatchlist = (title, platform, gamePlatforms) => {
    const current = new Set(savedPlatforms[title] || []);
    const wasSavedAtAll = current.size > 0;

    if (platform) {
      // A specific platform is in context — toggle just that one. Wishlisting
      // a game on PS5 must not affect (or read as affecting) its Xbox
      // entry, and vice versa, per the explicit ask this was built for.
      if (current.has(platform)) current.delete(platform);
      else current.add(platform);
    } else if (current.size > 0) {
      // No specific platform, already saved for at least one — treat this as
      // "un-watchlist the whole game" (e.g. the Watchlist screen's own heart,
      // which has no filter concept), same as the old single-toggle behavior.
      current.clear();
    } else {
      // No specific platform, not saved yet — default to every platform the
      // game is currently listed on, matching the old "just save the title"
      // behavior when there's no more specific intent to honor.
      (gamePlatforms || ALL_PLATFORM_KEYS).forEach((p) => current.add(p));
    }

    const willBeSavedAtAll = current.size > 0;

    setSavedPlatforms((prev) => {
      const next = { ...prev };
      if (current.size > 0) next[title] = [...current];
      else delete next[title];
      return next;
    });

    // Reminders stay one-per-title, not one-per-platform — only start/clear
    // one on a genuine add-from-nothing or remove-to-nothing. Toggling a
    // second platform on an already-saved title (or removing one of several)
    // leaves the existing reminder lead-time alone.
    if (!wasSavedAtAll && willBeSavedAtAll) {
      setReminders((r) => ({ ...r, [title]: r[title] || 'release_day' }));
      // ADDED (item 36 — in-app review prompt): only checked on a genuine
      // new add (this specific title going from unsaved to saved), not on
      // every platform toggle of an already-saved title — "3rd-5th game
      // added" means distinct titles, matching what the Watchlist tab's own
      // badge counts. resolvedWatchlistCount (not saved.size) so this reads
      // the same "does this still resolve to a real, current game" count
      // shown everywhere else, computed against `saved` plus this title
      // since the `saved` Set itself won't reflect this add until the
      // setSavedPlatforms update above actually commits.
      maybeRequestReview(resolvedWatchlistCount(new Set([...saved, title]), games)).catch(() => {});
    } else if (wasSavedAtAll && !willBeSavedAtAll) {
      setReminders((r) => {
        const { [title]: _, ...rest } = r;
        return rest;
      });
      // A title actually leaving the watchlist (not just losing one of
      // several platforms) has no more use for its last-known-data
      // snapshot — see savedGameSnapshots above. Swipe-to-remove's Undo
      // (restoreWatchlistEntry) re-adds the title to `saved`, which the
      // sync effect above will naturally re-populate from live data on its
      // own — no special-casing needed there.
      setSavedGameSnapshots((s) => {
        const { [title]: _, ...rest } = s;
        return rest;
      });
    }

    // Track the platform this title's single reminder should follow the
    // date of — the most recently explicitly-added platform. Only updated on
    // a genuine add of a specific platform (not a removal, and not the
    // "add every platform" no-context default, which has no one platform to
    // prefer over the others).
    if (platform && current.has(platform)) {
      setPlatformContext((p) => ({ ...p, [title]: platform }));
    }
    // Actual notification scheduling/cancellation is handled by the
    // syncReminderSchedule effect above, reacting to the `saved`/
    // `platformContext` change.
  };

  const setReminderLead = (title, key) => {
    setReminders((r) => ({ ...r, [title]: key }));
  };

  // ADDED (item 41 — Add to Calendar button state fix): called once
  // addGameReleaseToCalendar actually succeeds (see
  // app/game/[title].js) — see calendarAdded's own comment above for why
  // this persists past a later un-watchlist.
  const markCalendarAdded = (title) => {
    setCalendarAdded((c) => ({ ...c, [title]: true }));
  };

  // ADDED 2026-08-20: used only by the Watchlist tab's swipe-to-remove Undo
  // action (see components/SwipeableGameCard.js / app/(tabs)/watchlist.js).
  // toggleWatchlist has no way to know what a title's exact prior state was
  // once it's already been removed — calling it again would just re-derive
  // a fresh "every platform" default, not necessarily what was actually
  // there before (e.g. a title saved on PS5 only). This restores the exact
  // platform list, reminder lead time, and reminder-tracking platform from a
  // snapshot the caller took right before removing it. Deliberately narrow —
  // not exposed as a general-purpose setter anywhere else in the app.
  const restoreWatchlistEntry = (title, snapshot) => {
    if (!snapshot || !snapshot.platforms || snapshot.platforms.length === 0) return;
    setSavedPlatforms((prev) => ({ ...prev, [title]: snapshot.platforms }));
    if (snapshot.reminderLead) {
      setReminders((r) => ({ ...r, [title]: snapshot.reminderLead }));
    }
    if (snapshot.platformContext) {
      setPlatformContext((p) => ({ ...p, [title]: snapshot.platformContext }));
    }
  };

  const value = useMemo(
    () => ({
      saved,
      savedPlatforms,
      toggleWatchlist,
      restoreWatchlistEntry,
      reminders,
      setReminderLead,
      platformContext,
      savedGameSnapshots,
      calendarAdded,
      markCalendarAdded,
      preferredPlatforms,
      setPreferredPlatforms,
      preferredGenres,
      setPreferredGenres,
      weeklyDigestEnabled,
      setWeeklyDigestEnabled,
      // ADDED 2026-09-08 (item 29 fix): exposed so a screen that wants to
      // seed its own local state from preferredPlatforms/preferredGenres
      // (currently just the Calendar screen, see app/(tabs)/index.js) can
      // wait for this async AsyncStorage hydration to actually finish
      // before reading them, instead of capturing whatever value happened
      // to be in context at that screen's own first render — see the
      // Calendar screen's comment for the full story on why that mattered.
      hydrated,
    }),
    [saved, savedPlatforms, reminders, platformContext, savedGameSnapshots, calendarAdded, preferredPlatforms, preferredGenres, weeklyDigestEnabled, hydrated]
  );

  return <WatchlistContext.Provider value={value}>{children}</WatchlistContext.Provider>;
}

export function useWatchlist() {
  const ctx = useContext(WatchlistContext);
  if (!ctx) throw new Error('useWatchlist must be used inside WatchlistProvider');
  return ctx;
}
