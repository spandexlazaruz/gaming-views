...
export async function syncReminderSchedule(saved, reminders, games, leadOptions) {
  ...
  for (const title of savedTitles) {
    ...
    if (!lead) continue;
    await scheduleGameReminder(game, lead.days);
  }
}
...
export async function scheduleWeeklyDigest(saved, games) {
